// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using WS_CMVC_Demo.Data;

#nullable disable

namespace WS_CMVC_Demo.Migrations
{
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20220906205724_BadgeService_CheckByPackage")]
    partial class BadgeService_CheckByPackage
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "6.0.8")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder, 1L, 1);

            modelBuilder.Entity("WS_CMVC_Demo.Models.ApplicationRole", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Description")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("NormalizedName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedName")
                        .IsUnique()
                        .HasDatabaseName("RoleNameIndex")
                        .HasFilter("[NormalizedName] IS NOT NULL");

                    b.ToTable("AspNetRoles", (string)null);
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.ApplicationUser", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("AccessFailedCount")
                        .HasColumnType("int");

                    b.Property<bool>("Agreement")
                        .HasColumnType("bit");

                    b.Property<DateTime?>("ArrivalDateTime")
                        .HasColumnType("datetime2");

                    b.Property<string>("ArrivalDetails")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("BankDetails")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("CompanyName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("CompetenceId")
                        .HasColumnType("int");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("CountryId")
                        .HasColumnType("int");

                    b.Property<DateTime?>("DepartureDateTime")
                        .HasColumnType("datetime2");

                    b.Property<string>("DepartureDetails")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<bool>("EmailConfirmed")
                        .HasColumnType("bit");

                    b.Property<bool>("ImportedUser")
                        .HasColumnType("bit");

                    b.Property<bool>("IsGeneralBankDetails")
                        .HasColumnType("bit");

                    b.Property<DateTime?>("LastSigInDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("LockoutEnabled")
                        .HasColumnType("bit");

                    b.Property<DateTimeOffset?>("LockoutEnd")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("MiddleName")
                        .HasMaxLength(128)
                        .HasColumnType("nvarchar(128)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("nvarchar(128)");

                    b.Property<string>("NormalizedEmail")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("NormalizedUserName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("PassportNumber")
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<string>("PasswordHash")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PhoneNumber")
                        .HasColumnType("nvarchar(450)");

                    b.Property<bool>("PhoneNumberConfirmed")
                        .HasColumnType("bit");

                    b.Property<DateTime>("RegisterDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("RegisteredHimself")
                        .HasColumnType("bit");

                    b.Property<Guid?>("RegisteredUserId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int?>("RussiaSubjectId")
                        .HasColumnType("int");

                    b.Property<string>("SecondName")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("SecurityStamp")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("TwoFactorEnabled")
                        .HasColumnType("bit");

                    b.Property<int?>("UserCategoryId")
                        .HasColumnType("int");

                    b.Property<string>("UserName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<int?>("UserSubcategoryId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("CompetenceId");

                    b.HasIndex("CountryId");

                    b.HasIndex("NormalizedEmail")
                        .HasDatabaseName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasDatabaseName("UserNameIndex")
                        .HasFilter("[NormalizedUserName] IS NOT NULL");

                    b.HasIndex("PhoneNumber")
                        .IsUnique()
                        .HasFilter("[PhoneNumber] IS NOT NULL");

                    b.HasIndex("RegisteredUserId");

                    b.HasIndex("RussiaSubjectId");

                    b.HasIndex("UserCategoryId");

                    b.HasIndex("UserSubcategoryId");

                    b.ToTable("AspNetUsers", (string)null);
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Badge.BadgeColor", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("ColorGraph")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.ToTable("BadgeColors");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Badge.BadgeService", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<bool>("CheckByPackage")
                        .HasColumnType("bit");

                    b.Property<string>("IcoUrl")
                        .HasMaxLength(128)
                        .HasColumnType("nvarchar(128)");

                    b.Property<int?>("Order")
                        .HasColumnType("int");

                    b.Property<TimeSpan?>("PeriodTime")
                        .HasColumnType("time");

                    b.Property<byte>("PeriodType")
                        .HasColumnType("tinyint");

                    b.Property<TimeSpan?>("RecommendedEndTime")
                        .HasColumnType("time");

                    b.Property<TimeSpan?>("RecommendedStartTime")
                        .HasColumnType("time");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("nvarchar(128)");

                    b.HasKey("Id");

                    b.ToTable("BadgeServices");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Badge.BadgeServiceApplicationRole", b =>
                {
                    b.Property<int>("BadgeServiceId")
                        .HasColumnType("int");

                    b.Property<Guid>("RoleId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("BadgeServiceId", "RoleId");

                    b.HasIndex("RoleId");

                    b.ToTable("BadgeServiceApplicationRoles");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Badge.BadgeServiceCheckup", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("BadgeServiceId")
                        .HasColumnType("int");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid>("CreateUserId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<byte>("Type")
                        .HasColumnType("tinyint");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("BadgeServiceId");

                    b.HasIndex("CreateUserId");

                    b.HasIndex("UserId");

                    b.ToTable("BadgeServiceCheckups");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Event", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<DateTime>("DateEnd")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("DateStart")
                        .HasColumnType("datetime2");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("nvarchar(128)");

                    b.HasKey("Id");

                    b.ToTable("Events");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.EventUser", b =>
                {
                    b.Property<Guid>("UserId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("UserSubcategoryEventId")
                        .HasColumnType("int");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid>("CreateUserId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("UserId", "UserSubcategoryEventId");

                    b.HasIndex("CreateUserId");

                    b.HasIndex("UserSubcategoryEventId");

                    b.ToTable("EventUsers");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.RoleClaim", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid>("RoleId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetRoleClaims", (string)null);
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.SentEmail", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("Body")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid>("CreateUserId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Destination")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ErrorMessage")
                        .HasColumnType("nvarchar(max)");

                    b.Property<byte>("Status")
                        .HasColumnType("tinyint");

                    b.Property<string>("Subject")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("CreateUserId");

                    b.ToTable("AspNetSentEmail");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.SentSms", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid>("CreateUserId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("DeliveredDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("DeliveredShortMessage")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("DeliveredStatus")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PhoneNumber")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("StatusCod")
                        .HasColumnType("int");

                    b.Property<string>("StatusDescription")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Text")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("CreateUserId");

                    b.ToTable("AspNetSentSms");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Service.Hotel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("Address")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<byte?>("CountOfStars")
                        .HasColumnType("tinyint");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("nvarchar(128)");

                    b.HasKey("Id");

                    b.ToTable("Hotels");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Service.HotelOption", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("Description")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("HotelId")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("nvarchar(128)");

                    b.Property<byte>("NumberOfSeats")
                        .HasColumnType("tinyint");

                    b.HasKey("Id");

                    b.HasIndex("HotelId");

                    b.ToTable("HotelOptions");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Service.Package", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("Description")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasMaxLength(128)
                        .HasColumnType("nvarchar(128)");

                    b.HasKey("Id");

                    b.ToTable("Packages");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Service.PackageService", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<DateTime>("FinishDate")
                        .HasColumnType("datetime2");

                    b.Property<int>("MinimalDaysCount")
                        .HasColumnType("int");

                    b.Property<int>("PackageId")
                        .HasColumnType("int");

                    b.Property<int>("ServiceId")
                        .HasColumnType("int");

                    b.Property<DateTime>("StartDate")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("PackageId");

                    b.HasIndex("ServiceId");

                    b.ToTable("PackageServices");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Service.Quota", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("AvailableNum")
                        .HasColumnType("int");

                    b.Property<int>("Cost")
                        .HasColumnType("int");

                    b.Property<DateTime>("FinishDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("FixedCost")
                        .HasColumnType("bit");

                    b.Property<int>("ServiceId")
                        .HasColumnType("int");

                    b.Property<DateTime>("StartDate")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("ServiceId");

                    b.ToTable("Quotas");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Service.Service", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int?>("BadgeServiceId")
                        .HasColumnType("int");

                    b.Property<string>("Description")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("HotelOptionId")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<int>("ServiceTypeId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("BadgeServiceId");

                    b.HasIndex("HotelOptionId");

                    b.HasIndex("ServiceTypeId");

                    b.ToTable("Services");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Service.ServiceType", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("nvarchar(128)");

                    b.Property<int?>("Order")
                        .HasColumnType("int");

                    b.Property<bool>("ShowDates")
                        .HasColumnType("bit");

                    b.HasKey("Id");

                    b.ToTable("ServiceTypes");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Service.UserPackageService", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<Guid>("CreateUserId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("EventId")
                        .HasColumnType("int");

                    b.Property<DateTime>("FinishDate")
                        .HasColumnType("datetime2");

                    b.Property<int>("PackageServiceId")
                        .HasColumnType("int");

                    b.Property<DateTime>("StartDate")
                        .HasColumnType("datetime2");

                    b.Property<byte>("Status")
                        .HasColumnType("tinyint");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("CreateUserId");

                    b.HasIndex("EventId");

                    b.HasIndex("PackageServiceId");

                    b.HasIndex("UserId");

                    b.ToTable("UserPackageServices");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Service.UserSubcategoryEventPackage", b =>
                {
                    b.Property<int>("PackageId")
                        .HasColumnType("int");

                    b.Property<int>("UserSubcategoryEventId")
                        .HasColumnType("int");

                    b.HasKey("PackageId", "UserSubcategoryEventId");

                    b.HasIndex("UserSubcategoryEventId");

                    b.ToTable("UserSubcategoryEventPackages");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.UserCategory", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("nvarchar(128)");

                    b.HasKey("Id");

                    b.ToTable("UserCategories");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.UserClaim", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserClaims", (string)null);
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.UserCompetence", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int?>("Order")
                        .HasColumnType("int");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("nvarchar(128)");

                    b.HasKey("Id");

                    b.ToTable("UserCompetences");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.UserCountry", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int?>("Order")
                        .HasColumnType("int");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("nvarchar(128)");

                    b.HasKey("Id");

                    b.ToTable("UserCountries");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.UserLogin", b =>
                {
                    b.Property<string>("LoginProvider")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ProviderKey")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ProviderDisplayName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("LoginProvider", "ProviderKey");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserLogins", (string)null);
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.UserRole", b =>
                {
                    b.Property<Guid>("UserId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("RoleId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("UserId", "RoleId");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetUserRoles", (string)null);
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.UserRussiaSubject", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int?>("Order")
                        .HasColumnType("int");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("nvarchar(128)");

                    b.HasKey("Id");

                    b.ToTable("UserRussiaSubjects");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.UserSubcategory", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<bool>("AutoPackage")
                        .HasColumnType("bit");

                    b.Property<int?>("BadgeColorId")
                        .HasColumnType("int");

                    b.Property<int?>("BadgeTextColorId")
                        .HasColumnType("int");

                    b.Property<int>("CategoryId")
                        .HasColumnType("int");

                    b.Property<string>("IncludeProperties")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("NotFreePackage")
                        .HasColumnType("bit");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("nvarchar(128)");

                    b.Property<string>("TitleForPrint")
                        .HasMaxLength(128)
                        .HasColumnType("nvarchar(128)");

                    b.HasKey("Id");

                    b.HasIndex("BadgeColorId");

                    b.HasIndex("BadgeTextColorId");

                    b.HasIndex("CategoryId");

                    b.ToTable("UserSubcategories");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.UserSubcategoryEvent", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("EventId")
                        .HasColumnType("int");

                    b.Property<int>("UserSubcategoryId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("EventId");

                    b.HasIndex("UserSubcategoryId");

                    b.ToTable("UserSubcategoryEvents");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.UserToken", b =>
                {
                    b.Property<Guid>("UserId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("LoginProvider")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Value")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("UserId", "LoginProvider", "Name");

                    b.ToTable("AspNetUserTokens", (string)null);
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.ApplicationUser", b =>
                {
                    b.HasOne("WS_CMVC_Demo.Models.UserCompetence", "Competence")
                        .WithMany()
                        .HasForeignKey("CompetenceId");

                    b.HasOne("WS_CMVC_Demo.Models.UserCountry", "Country")
                        .WithMany()
                        .HasForeignKey("CountryId");

                    b.HasOne("WS_CMVC_Demo.Models.ApplicationUser", "RegisteredUser")
                        .WithMany()
                        .HasForeignKey("RegisteredUserId");

                    b.HasOne("WS_CMVC_Demo.Models.UserRussiaSubject", "RussiaSubject")
                        .WithMany()
                        .HasForeignKey("RussiaSubjectId");

                    b.HasOne("WS_CMVC_Demo.Models.UserCategory", "UserCategory")
                        .WithMany()
                        .HasForeignKey("UserCategoryId");

                    b.HasOne("WS_CMVC_Demo.Models.UserSubcategory", "UserSubcategory")
                        .WithMany()
                        .HasForeignKey("UserSubcategoryId");

                    b.Navigation("Competence");

                    b.Navigation("Country");

                    b.Navigation("RegisteredUser");

                    b.Navigation("RussiaSubject");

                    b.Navigation("UserCategory");

                    b.Navigation("UserSubcategory");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Badge.BadgeServiceApplicationRole", b =>
                {
                    b.HasOne("WS_CMVC_Demo.Models.Badge.BadgeService", "BadgeService")
                        .WithMany("Roles")
                        .HasForeignKey("BadgeServiceId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("WS_CMVC_Demo.Models.ApplicationRole", "Role")
                        .WithMany("BadgeServices")
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("BadgeService");

                    b.Navigation("Role");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Badge.BadgeServiceCheckup", b =>
                {
                    b.HasOne("WS_CMVC_Demo.Models.Badge.BadgeService", "BadgeService")
                        .WithMany()
                        .HasForeignKey("BadgeServiceId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("WS_CMVC_Demo.Models.ApplicationUser", "Creator")
                        .WithMany()
                        .HasForeignKey("CreateUserId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("WS_CMVC_Demo.Models.ApplicationUser", "User")
                        .WithMany("BadgeServiceCheckups")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("BadgeService");

                    b.Navigation("Creator");

                    b.Navigation("User");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.EventUser", b =>
                {
                    b.HasOne("WS_CMVC_Demo.Models.ApplicationUser", "Creator")
                        .WithMany()
                        .HasForeignKey("CreateUserId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("WS_CMVC_Demo.Models.ApplicationUser", "User")
                        .WithMany("EventUsers")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("WS_CMVC_Demo.Models.UserSubcategoryEvent", "UserSubcategoryEvent")
                        .WithMany()
                        .HasForeignKey("UserSubcategoryEventId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("Creator");

                    b.Navigation("User");

                    b.Navigation("UserSubcategoryEvent");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.RoleClaim", b =>
                {
                    b.HasOne("WS_CMVC_Demo.Models.ApplicationRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.SentEmail", b =>
                {
                    b.HasOne("WS_CMVC_Demo.Models.ApplicationUser", "Creator")
                        .WithMany()
                        .HasForeignKey("CreateUserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Creator");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.SentSms", b =>
                {
                    b.HasOne("WS_CMVC_Demo.Models.ApplicationUser", "Creator")
                        .WithMany()
                        .HasForeignKey("CreateUserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Creator");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Service.HotelOption", b =>
                {
                    b.HasOne("WS_CMVC_Demo.Models.Service.Hotel", "Hotel")
                        .WithMany("HotelOptions")
                        .HasForeignKey("HotelId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Hotel");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Service.PackageService", b =>
                {
                    b.HasOne("WS_CMVC_Demo.Models.Service.Package", "Package")
                        .WithMany("PackageServices")
                        .HasForeignKey("PackageId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("WS_CMVC_Demo.Models.Service.Service", "Service")
                        .WithMany("PackageServices")
                        .HasForeignKey("ServiceId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Package");

                    b.Navigation("Service");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Service.Quota", b =>
                {
                    b.HasOne("WS_CMVC_Demo.Models.Service.Service", "Service")
                        .WithMany("Quotas")
                        .HasForeignKey("ServiceId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Service");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Service.Service", b =>
                {
                    b.HasOne("WS_CMVC_Demo.Models.Badge.BadgeService", "BadgeService")
                        .WithMany()
                        .HasForeignKey("BadgeServiceId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("WS_CMVC_Demo.Models.Service.HotelOption", "HotelOption")
                        .WithMany("Services")
                        .HasForeignKey("HotelOptionId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("WS_CMVC_Demo.Models.Service.ServiceType", "ServiceType")
                        .WithMany("Services")
                        .HasForeignKey("ServiceTypeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("BadgeService");

                    b.Navigation("HotelOption");

                    b.Navigation("ServiceType");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Service.UserPackageService", b =>
                {
                    b.HasOne("WS_CMVC_Demo.Models.ApplicationUser", "Creator")
                        .WithMany()
                        .HasForeignKey("CreateUserId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("WS_CMVC_Demo.Models.Event", "Event")
                        .WithMany("UserPackageServices")
                        .HasForeignKey("EventId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("WS_CMVC_Demo.Models.Service.PackageService", "PackageService")
                        .WithMany("UserPackageServices")
                        .HasForeignKey("PackageServiceId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("WS_CMVC_Demo.Models.ApplicationUser", "User")
                        .WithMany("PackageServices")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Creator");

                    b.Navigation("Event");

                    b.Navigation("PackageService");

                    b.Navigation("User");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Service.UserSubcategoryEventPackage", b =>
                {
                    b.HasOne("WS_CMVC_Demo.Models.Service.Package", "Package")
                        .WithMany("UserSubcategoryEventPackages")
                        .HasForeignKey("PackageId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("WS_CMVC_Demo.Models.UserSubcategoryEvent", "UserSubcategoryEvent")
                        .WithMany("UserSubcategoryEventPackages")
                        .HasForeignKey("UserSubcategoryEventId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Package");

                    b.Navigation("UserSubcategoryEvent");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.UserClaim", b =>
                {
                    b.HasOne("WS_CMVC_Demo.Models.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.UserLogin", b =>
                {
                    b.HasOne("WS_CMVC_Demo.Models.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.UserRole", b =>
                {
                    b.HasOne("WS_CMVC_Demo.Models.ApplicationRole", "Role")
                        .WithMany("Users")
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("WS_CMVC_Demo.Models.ApplicationUser", "User")
                        .WithMany("Roles")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Role");

                    b.Navigation("User");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.UserSubcategory", b =>
                {
                    b.HasOne("WS_CMVC_Demo.Models.Badge.BadgeColor", "BadgeColor")
                        .WithMany()
                        .HasForeignKey("BadgeColorId");

                    b.HasOne("WS_CMVC_Demo.Models.Badge.BadgeColor", "BadgeTextColor")
                        .WithMany()
                        .HasForeignKey("BadgeTextColorId");

                    b.HasOne("WS_CMVC_Demo.Models.UserCategory", "Category")
                        .WithMany("Subcategories")
                        .HasForeignKey("CategoryId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("BadgeColor");

                    b.Navigation("BadgeTextColor");

                    b.Navigation("Category");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.UserSubcategoryEvent", b =>
                {
                    b.HasOne("WS_CMVC_Demo.Models.Event", "Event")
                        .WithMany("UserSubcategoryEvents")
                        .HasForeignKey("EventId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("WS_CMVC_Demo.Models.UserSubcategory", "UserSubcategoryCategory")
                        .WithMany("Events")
                        .HasForeignKey("UserSubcategoryId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Event");

                    b.Navigation("UserSubcategoryCategory");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.UserToken", b =>
                {
                    b.HasOne("WS_CMVC_Demo.Models.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.ApplicationRole", b =>
                {
                    b.Navigation("BadgeServices");

                    b.Navigation("Users");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.ApplicationUser", b =>
                {
                    b.Navigation("BadgeServiceCheckups");

                    b.Navigation("EventUsers");

                    b.Navigation("PackageServices");

                    b.Navigation("Roles");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Badge.BadgeService", b =>
                {
                    b.Navigation("Roles");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Event", b =>
                {
                    b.Navigation("UserPackageServices");

                    b.Navigation("UserSubcategoryEvents");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Service.Hotel", b =>
                {
                    b.Navigation("HotelOptions");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Service.HotelOption", b =>
                {
                    b.Navigation("Services");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Service.Package", b =>
                {
                    b.Navigation("PackageServices");

                    b.Navigation("UserSubcategoryEventPackages");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Service.PackageService", b =>
                {
                    b.Navigation("UserPackageServices");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Service.Service", b =>
                {
                    b.Navigation("PackageServices");

                    b.Navigation("Quotas");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.Service.ServiceType", b =>
                {
                    b.Navigation("Services");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.UserCategory", b =>
                {
                    b.Navigation("Subcategories");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.UserSubcategory", b =>
                {
                    b.Navigation("Events");
                });

            modelBuilder.Entity("WS_CMVC_Demo.Models.UserSubcategoryEvent", b =>
                {
                    b.Navigation("UserSubcategoryEventPackages");
                });
#pragma warning restore 612, 618
        }
    }
}
